...
def update_files(file_list, flash_drive):
    ...
    print("Введите номера файлов для обновления через запятую (или 'в'/'0' для копирования всех, или 'н' для пропуска): ")
    choice = input().strip()

    # Обработка выбора
    if not choice or choice.lower() == 'н':  # Если пользователь ввёл 'н' или оставил ввод пустым
        print("Копирование файлов пропущено.")
        return

    if choice.lower() in {'в', '0'}:  # Проверяем ввод: 'в', 'В', '0'
